...
namespace ZLGCANDemo
{
    ...
    class recvdatathread
    {
        ...
        private RecvCANDataEventHandler OnRecvCANDataEvent;
        private RecvFDDataEventHandler OnRecvFDDataEvent;
        ...
        {
            ZCAN_Receive_Data[] can_data = new ZCAN_Receive_Data[1000000];
            ZCAN_ReceiveFD_Data[] canfd_data = new ZCAN_ReceiveFD_Data[1000000];
            uint len;
            while (m_bStart)
            {
                lock (locker)
                {
                    ...
                    if (len > 0)
                    {
                        ...
                        for (int i = 0; i < len; ++i)
                        {
                            ...
                            //Console.WriteLine(canfd_data[i].timestamp+">");
                        }
